...
function loadAboutSystemTab() {
  ...
  const currentChannel = isBeta ? "beta" : "stable";
  const otherChannel = isBeta ? "stable" : "beta";
  const otherURL = isBeta
  ...
  container.innerHTML = `
    ...
    <div>
      <button id="load-hardcoded-btn" class="btn btn-primary">
        Load Hardcoded JSON
      </button>
    ...
    <div style="margin-top:12px;">
      <button id="toggle-devmode-btn" class="btn btn-primary">
        ${localStorage.getItem("devMode")==="true"
          ? "Disable Dev Mode"
          : "Enable Dev Mode"}
      </button>
      <button id="switch-channel-btn" class="btn btn-secondary">
  ...
  function renderVersionData(data) {
    const info = document.getElementById("about-details");
    localStorage.setItem("versionData", JSON.stringify(data));

    let entry;
    if (Array.isArray(data)) {
      entry = data.find(i => i.Channel.toLowerCase() === currentChannel);
    } else {
      entry = (data.Channel && data.Channel.toLowerCase() === currentChannel)
        ? data
        : null;
    }

    if (!entry) {
      info.innerHTML = `<p style="color:#f87171;">
        <strong>Error:</strong> No version info for "${currentChannel}".
      </p>`;
      return;
    }
    ...
    info.innerHTML = `
      ...
      <p><strong>Log:</strong><br>${entry.Log.replace(/\n/g, "<br>")}</p>
    `;
  }

  // Button handlers
  document.getElementById("load-hardcoded-btn")
    .addEventListener("click", () => renderVersionData(HARDCODED_VERSION_JSON));
  ...
  document.getElementById("import-json-btn")
    .addEventListener("click", () => {
      ...
      input.onchange = e => {
        ...
        reader.onload = evt => {
          try {
            renderVersionData(JSON.parse(evt.target.result));
          } catch (err) {
  ...
  document.getElementById("toggle-devmode-btn")
    .addEventListener("click", toggleDevMode);
  ...
  // Auto-render last-used JSON
  const stored = localStorage.getItem("versionData");
  if (stored) {
    try {
      renderVersionData(JSON.parse(stored));
    } catch {}
  }
}

// Initialize after DOM is ready
document.addEventListener("DOMContentLoaded", loadAboutSystemTab);
...
const HARDCODED_VERSION_JSON = [
  {
    "Name": "Jxo OS-Offline",
    "Channel": "stable",
    "Ver": "8.3.2",
    "Log": "New about system tab in settings and dev support."
  },
    {
    "Name": "Jxo OS-beta-Offline",
    "Channel": "beta",
    "Ver": "8.3.2",
    "Log": "Working version"
  }
